        ...
        switch (peptideResultOutputType){
            ...
            case TRANSITIONS:
                ...
                for(String transition : transitions){
                    ...
                    ExpIReplicateToRecordsMapper expReplicateToRecords = new ExpIReplicateToRecordsMapper();
            ...
            case SUMMED:
                ...
                logWriter.println("    computing summed transition(s) associated LLOQ estimation...");
                System.out.println("    computing summed transition(s) associated LLOQ estimation...");
                ExpICalibrationPointToRecordsMapper pointToRecordsMapper = 
                            new ExpICalibrationPointToRecordsMapper();
    ...
    private CoefficientOfVariation computeLLOQ(LinkedList<CoefficientOfVariation> coeffs, 
        ...
        if(coeffs.size() > 0){
            ...
            if(coeffsBelow20.size() > 0){
                ...
                // Using the lowest spiked-in dilution of measures/points with cv's < 20%
                // double lowestDilution = pointToDilutionMap.get(lloq.getId()); // uses the spiked-in dilution at this level
                ...
                double lowestAveragedConc = lloq.getMean(); 
                for(CoefficientOfVariation coeffBelow20 : coeffsBelow20){
                    double averagedConc = coeffBelow20.getMean();                    
                    if(averagedConc < lowestAveragedConc){
                        lloq = coeffBelow20;
                    }
                }
            }
